...
// #define DEBUG_EIJIS_OCTAGON
// #define DEBUG_EIJIS_OCTAGON_CUSHION_TRACE
...
public class AdvancedPhysicsManager_E2 : UdonSharpBehaviour
{
#if EIJIS_MANY_BALLS
    public string PHYSICSNAME = "<color=#FFD700>Advanced V0.5K (Octagon)</color>";
#else
...
#if EIJIS_OCTAGON_TABLE
    float k_TABLE_HW_RATE;
    float k_TABLE_CORNER_ADJUST;
#endif
...
#endif
    private byte pocket_mask_sides = 0x18; // E,W = 4,5
                ...
                if ((ball_bit & sn_pocketed) == 0U)
                {
                    ...
                    if (balls_V[i] != Vector3.zero || balls_W[i] != Vector3.zero)
                    {
                        ...
                        if (isCarom)
                        {
                            ...
                            hitCushion = _phy_ball_table_carom_octagon(i);
                        }
                        else
                        {
    ...
    */
...
#if EIJIS_OCTAGON_TABLE
        k_TABLE_HW_RATE = k_TABLE_HEIGHT / k_TABLE_WIDTH;
        // float theta = Mathf.Atan(k_TABLE_HW_RATE);
        // float tan = Mathf.Tan(k_TABLE_HW_RATE);
        // // k_TABLE_CORNER_ADJUST =  k_BALL_RADIUS / Mathf.Cos(theta);
        // // k_TABLE_CORNER_ADJUST =  k_BALL_RADIUS * tan;
        // k_TABLE_CORNER_ADJUST =  k_BALL_DIAMETRE * tan;
        k_TABLE_CORNER_ADJUST = 0.07f; // 0.08f;
#if DEBUG_EIJIS_OCTAGON
        table._LogInfo($"k_TABLE_WIDTH = {k_TABLE_WIDTH}, k_TABLE_HEIGHT = {k_TABLE_HEIGHT}, k_TABLE_HW_RATE = {k_TABLE_HW_RATE}");
        table._LogInfo($"Mathf.Tan(k_TABLE_HW_RATE) = {Mathf.Tan(k_TABLE_HW_RATE)}, k_BALL_RADIUS = {k_BALL_RADIUS}, k_TABLE_CORNER_ADJUST = {k_TABLE_CORNER_ADJUST}");
        // table._LogInfo($"theta = {theta * Mathf.Rad2Deg}(deg), Mathf.Cos(theta) = {Mathf.Cos(theta)}");
        table._LogInfo($"k_POCKET_WIDTH_CORNER = {k_POCKET_WIDTH_CORNER}, k_POCKET_HEIGHT_CORNER = {k_POCKET_HEIGHT_CORNER}");
#endif
#endif
...
#if DEBUG_EIJIS_OCTAGON
        table._LogInfo($"k_vE = ({k_vE.x},{k_vE.y},{k_vE.z}), k_vE2 = ({k_vE2.x},{k_vE2.y},{k_vE2.z})");
#endif
...
#if DEBUG_EIJIS_OCT_POCKETS || DEBUG_EIJIS_OCTAGON
...
#if DEBUG_EIJIS_OCTAGON
        table._LogInfo($"k_pO = ({k_pO.x},{k_pO.y},{k_pO.z})");
        table._LogInfo($"k_pR = ({k_pR.x},{k_pR.y},{k_pR.z})");
#endif
...
#endif
#if DEBUG_EIJIS_OCTAGON_CUSHION_TRACE
        if (id == 0) table._LogInfo($"P00 enable_pockets = {enable_pockets:X2}, pocket_mask_by_sign = {pocket_mask_by_sign:X2}, pocket_mask_corners = {pocket_mask_corners:X2}, enable_pockets & pocket_mask_by_sign = {(enable_pockets & pocket_mask_by_sign):X2}, enable_pockets & pocket_mask_corners = {(enable_pockets & pocket_mask_corners):X2}");
#endif
#if EIJIS_DISABLE_POCKET
        if (!isCarom && 0 != (enable_pockets & pocket_mask_by_sign))
#else        
        if (!isCarom)
#endif
        {
#if DEBUG_EIJIS_OCTAGON_CUSHION_TRACE
            if (id == 0) table._LogInfo($"P01 (absA - k_vE).sqrMagnitude < k_INNER_RADIUS_CORNER_SQ && (absA - k_vE2).sqrMagnitude < k_INNER_RADIUS_CORNER_SQ2 = {(absA - k_vE).sqrMagnitude} < {k_INNER_RADIUS_CORNER_SQ} && {(absA - k_vE2).sqrMagnitude} < {k_INNER_RADIUS_CORNER_SQ2}");
#endif
#if EIJIS_DISABLE_POCKET
...
#endif
            {
                inPocketBounds = true;
#if DEBUG_EIJIS_OCTAGON_CUSHION_TRACE
                if (id == 0) table._LogInfo($"P02 A.y < -k_BALL_RADIUS = {A.y} < - {k_BALL_RADIUS}");
#endif
...
#if DEBUG_EIJIS_OCTAGON_CUSHION_TRACE
            if (id == 0) table._LogInfo($"P10 (absA - k_vFF).sqrMagnitude < k_INNER_RADIUS_CORNER_SQ && (absA - k_vFF2).sqrMagnitude < k_INNER_RADIUS_CORNER_SQ2 = {(absA - k_vFF).sqrMagnitude} < {k_INNER_RADIUS_CORNER_SQ} && {(absA - k_vFF2).sqrMagnitude} < {k_INNER_RADIUS_CORNER_SQ2}");
#endif
#if EIJIS_DISABLE_POCKET
...
#if DEBUG_EIJIS_OCTAGON_CUSHION_TRACE
        if (id == 0) table._LogInfo($"P20 absA = ({absA.x}, {absA.z}),  tableEdge = ({tableEdge.x}, {tableEdge.y})");
#endif
    ...
    bool _phy_ball_table_carom_octagon(int id)
    {
        if (balls_P[id].y > k_RAIL_HEIGHT_UPPER)
        {
            //ball is above rail
        ...
        newPos = Vector3.Scale(newPos, _sign_pos);
        // Setup major regions
        Vector3 N = Vector3.zero, a_to_v;

        Vector3 newVel = balls_V[id];
        Vector3 newAngVel = balls_W[id];

#if DEBUG_EIJIS_OCTAGON_CUSHION_TRACE
        if (id == 0) table._LogInfo($"00 newPos.z > ((-k_TABLE_HW_RATE) * newPos.x) + (k_TABLE_HEIGHT * 2 - k_POCKET_HEIGHT_CORNER - k_TABLE_CORNER_ADJUST) = {newPos.z} > {((-k_TABLE_HW_RATE) * newPos.x) + (k_TABLE_HEIGHT * 2 - k_POCKET_HEIGHT_CORNER - k_TABLE_CORNER_ADJUST)} ( {((-k_TABLE_HW_RATE) * newPos.x)} + {k_TABLE_HEIGHT * 2 - k_POCKET_HEIGHT_CORNER - k_TABLE_CORNER_ADJUST})");
#endif
        // if (newPos.z > ((-k_TABLE_HW_RATE) * newPos.x) + ((k_TABLE_HEIGHT * 2) - (k_POCKET_HEIGHT_CORNER + 0.1f)))
        if (newPos.z > ((-k_TABLE_HW_RATE) * newPos.x) + ((k_TABLE_HEIGHT * 2) - (k_POCKET_HEIGHT_CORNER + k_TABLE_CORNER_ADJUST)))
        {
            // newPos.x = caromEdgeX; // todo
            N = new Vector3(-k_TABLE_WIDTH, 0, -k_TABLE_HEIGHT);
