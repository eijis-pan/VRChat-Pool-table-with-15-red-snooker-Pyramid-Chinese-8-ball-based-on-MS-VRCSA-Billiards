...
#define EIJIS_MSVRCSA_PHYSICS_SUPPORT
#define EIJIS_MSVRCSA_TO_CBC

// #define EIJIS_ADJUST_PHYSICS_PARAM
#define EIJIS_ADJUST_PHYSICS_PARAM_EDITOR
...
public class AdvancedPhysicsManager_V06X : UdonSharpBehaviour
{
#if EIJIS_MANY_BALLS
#if EIJIS_ADJUST_PHYSICS_PARAM
    public string PHYSICSNAME = "<color=#00EEFF>Advanced V0.6X (max32balls)</color>";
#else
    public string PHYSICSNAME = "<color=#26FF00>Advanced V0.6X (max32balls)</color>";
#endif
#else
   ...
   private  float k_MAX_DELTA =  0.05f; // Private Const Float 0.05f max time to process per frame on quest (~4)
...
#if EIJIS_ADJUST_PHYSICS_PARAM_EDITOR
    [Header("Ball <-> Table Variables ")]
    public float k_F_SLIDE = 0.2f;
    public float K_F_CUSHION = 0.2f;
#endif
#if !EIJIS_ADJUST_PHYSICS_PARAM_EDITOR
#if EIJIS_ADJUST_PHYSICS_PARAM
    [NonSerializedAttribute] public float k_F_SLIDE = 0.38f;                                                         // Friction coefficient of sliding          (Ball-Table)    [Update Velocity]
#else
...
#if !EIJIS_ADJUST_PHYSICS_PARAM_EDITOR
#if EIJIS_ADJUST_PHYSICS_PARAM
    [NonSerializedAttribute] public float K_F_CUSHION = 0.12f;
#else
    ...
    {
#if EIJIS_MANY_BALLS
        for (int i = id; i < BilliardsModule.MAX_BALLS; i++)
#else
        for (int i = id; i < 16; i++)
#endif
        {
...
#if EIJIS_MANY_BALLS
            for (int j = 0; j < BilliardsModule.MAX_BALLS; j++)
#else
            for (int j = 0; j < 16; j++)
#endif
            {
...
#if EIJIS_MSVRCSA_PHYSICS_SUPPORT
        k_INNER_RADIUS_CORNER2 = k_INNER_RADIUS_CORNER;
#else
...
#if EIJIS_MSVRCSA_PHYSICS_SUPPORT
        k_INNER_RADIUS_SIDE2 = k_INNER_RADIUS_SIDE;
#else
