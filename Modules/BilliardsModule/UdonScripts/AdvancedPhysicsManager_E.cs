...
// #define DEBUG_EIJIS_CIRCLE_CUSHION_TRACE
...
public class AdvancedPhysicsManager_E : UdonSharpBehaviour
{
#if EIJIS_MANY_BALLS
    public string PHYSICSNAME = "<color=#FFD700>Advanced V0.5K (Circle)</color>";
#else
                ...
                if ((ball_bit & sn_pocketed) == 0U)
                {
                    ...
                    if (balls_V[i] != Vector3.zero || balls_W[i] != Vector3.zero)
                    {
                        ...
                        if (isCarom)
                        {
                            ...
                            hitCushion = _phy_ball_table_carom_circle(i);
                        }
                        else
                        {
    ...
    */
    ...
    bool _phy_ball_table_carom_circle(int id)
    {
        if (balls_P[id].y > k_RAIL_HEIGHT_UPPER)
        {
            //ball is above rail
...
#if DEBUG_EIJIS_CIRCLE_CUSHION_TRACE
        if (id == 0) table._LogInfo($"00 newPos.sqrMagnitude > caromEdgeX * caromEdgeZ = {newPos.sqrMagnitude} > {caromEdgeX} * {caromEdgeZ} ({caromEdgeX * caromEdgeZ})");
#endif
        if (newPos.sqrMagnitude > caromEdgeX * caromEdgeZ)
        {
            a_to_v = Vector3.zero - newPos;
            N = a_to_v.normalized;
